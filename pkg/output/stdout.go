...
func (s *Stdoutput) Progress(status ffuf.Progress) {
	...
	fmt.Fprintf(os.Stderr, "%s:: Progress: [%d/%d] %d%s :: Job [%d/%d] :: %d req/sec :: Duration: [%d:%02d:%02d] :: Errors: %d ::", TERMINAL_CLEAR_LINE, status.ReqCount, status.ReqTotal, percentage, str, status.QueuePos, status.QueueTotal, reqRate, hours, mins, secs, status.ErrorCount)
}
